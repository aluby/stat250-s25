...
- $\hat{\mu_1} = \bar{X}$
- $\hat{\mu_2} = \widetilde{X}$
- $\hat{\mu_3} = \frac{X_{\max} - X_{\min}}{2}$

## Aside: estimators are also random variables

$X_1, ..., X_n \sim F_x(\theta)$ and each $\hat{\theta} = g(X_1, ..., X_n)$ is a function of the data

. . . 

This means that each $\hat\theta$ is itself a random variable, and so it has: 

- a pdf $f_\hat{\theta}$
- an expectation $E(\hat\theta)$
- a variance $\text{Var}(\hat{\theta})$
...
If $\hat{\theta}(X)$ is an estimator of $\theta$, then the bias of the estimator is equal to

$$\text{Bias}(\hat\theta) = E(\hat \theta(X)) - \theta$$

*Note:* the expected value is computed from the sampling distribution of $\hat{\theta}(X)$
:::
...
How much variabiliity does the estimator have **around $\theta$**?
...
## Example: Unif(0, $\theta$) distribution

Your task is to compare the estimators 

$$\hat{\theta}_{MLE} = X_\max \hspace{1in} \hat{\theta}_{MoM} = 2\bar{X}$$ 
...
(d) When does $\hat{\theta}_{MLE}$ "beat"  $\hat{\theta}_{MoM}$ in terms of MSE?
:::
...
For two unbiased estimators, $\hat \theta_1$ is more **efficient** than $\hat \theta_2$ if 

$$\text{Var}(\hat \theta_1) < \text{Var}(\hat \theta_2) \iff \text{SE}(\hat \theta_1) < \text{SE}(\hat \theta_2)$$
:::
